...
/**
 * ローカルでのデバッグ用に，メモリ上 or リソース上の値を返す{@link com.android.volley.toolbox.HttpStack}.
 */
class FakeHttpStack implements HttpStack {
    ...
    private HttpEntity createEntity(Request request) throws UnsupportedEncodingException {
        ...
        if (resourceId == 0) {
            DoyaLogger.warn("No fake file named ", resourceName,
            ...
            try {
                ...
                DoyaLogger.error("error reading " + resourceName, e);
            }
        }
        // 適切なリソースが無いので，適当に返す
    ...
    private String constructFakeResponseFileName(Request request) {
        ...
        String apiName = reqUrl.substring(DoyaAPI.BASE.length()).split("/", 2)[0];
